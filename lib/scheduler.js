...
class SchedulerEntry {
	...
	/**
	* @param {Nodest.Scheduler} scheduler The scheduler instance this entry belongs to
	*/
	constructor(scheduler) {
		...
		this.log = Nodest.Log.scoped("scheduler");
	}
	...
	*/
	perform(path) {
		var Task = require(this.scheduler.pathContext.resolve(path));
		this.task = new Task();
	}
	...
	*/
	exec() {
		...
		this.task && this.task.exec(this.scheduler.app, () => {});
	...
	*/
	start() {
		...
		let timeouts = this.times.map(v => {
			...
			this.log.debug(`${this.task.constructor.name} will execute in ${Math.round(offset/1000)} seconds`);
...
* @param {Nodest.App} app Application object
*/
class Scheduler {

	constructor(pathContext, app) {

		this.pathContext = pathContext;
		this.app = app;
		this.entries = global_entries;
